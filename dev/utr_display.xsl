...
<xsl:template match="/characterMapping">
    ...
    <style type="text/css">
        ...
        .nochar {font: 200%, '<xsl:value-of select="$char_font"/>'; margin: 0; padding: 0; text-align: center; color: #F8F8FF}
...
<!--    <h2>Chart</h2> -->
    ...
    <h2>Warnings</h2>
        <xsl:apply-templates select="//a|//fbu|//fub|//range|//class-ref" mode="errors"/>
    <h2>History</h2>
...
<!-- pink: none or multiple reverse mappings for corresponding Unicode -->
...
<xsl:template match="a" mode="errors">
    <xsl:call-template name="test-bctxt"/>
    <xsl:call-template name="test-uctxt"/>
</xsl:template>
...
<xsl:template name="test-bctxt">
    <xsl:param name="b" select="@b"/>
    <xsl:param name="u" select="@u"/>
    <xsl:if test="not(@bactxt) and not(@bbctxt) and not(contains($u, ' '))">
        <xsl:if test="not(key('uni-key', $u)|local:nodeset($ranges)/a[@u=$u])">
            <p>No reverse mapping for U+<xsl:value-of select="$u"/> mapped from 0x<xsl:value-of select="$b"/></p>
        </xsl:if>
        <xsl:if test="count(key('uni-key', $u)|local:nodeset($ranges)/a[@u=$u]) > 1">
            <p>Multiple mapping from U+<xsl:value-of select="$u"/> to at least 0x<xsl:value-of select="$b"/></p>
        </xsl:if>
    </xsl:if>
    <xsl:call-template name="test-ctxt">
    	<xsl:with-param name="ctxt" select="@bbctxt"/>
    	<xsl:with-param name="b" select="$b"/>
    	<xsl:with-param name="u" select="$u"/>
    </xsl:call-template>
    <xsl:call-template name="test-ctxt">
    	<xsl:with-param name="ctxt" select="@bactxt"/>
    	<xsl:with-param name="b" select="$b"/>
    	<xsl:with-param name="u" select="$u"/>
    </xsl:call-template>
</xsl:template>

<xsl:template name="test-uctxt">
    <xsl:param name="b" select="@b"/>
    <xsl:param name="u" select="@u"/>
    <xsl:if test="not(@uactxt) and not(@ubctxt) and not(contains($b, ' '))">
        <xsl:if test="not(key('byte-key', $b)|local:nodeset($ranges)/a[@b=$b])">
            <p>No reverse mapping for 0x<xsl:value-of select="$b"/> mapped from U+<xsl:value-of select="$u"/></p>
        </xsl:if>
        <xsl:if test="count(key('byte-key', $b)|local:nodeset($ranges)/a[@b=$b]) > 1">
            <p>Multiple mapping from 0x<xsl:value-of select="$b"/> to at least U+<xsl:value-of select="$u"/></p>
        </xsl:if>
    </xsl:if>
    <xsl:call-template name="test-ctxt">
    	<xsl:with-param name="ctxt" select="@ubctxt"/>
    	<xsl:with-param name="b" select="$b"/>
    	<xsl:with-param name="u" select="$u"/>
    </xsl:call-template>
    <xsl:call-template name="test-ctxt">
    	<xsl:with-param name="ctxt" select="@uactxt"/>
    	<xsl:with-param name="b" select="$b"/>
    	<xsl:with-param name="u" select="$u"/>
    </xsl:call-template>
</xsl:template>

<xsl:template name="test-ctxt">
    <xsl:param name="ctxt"/>
    <xsl:param name="b" select="@b"/>
    <xsl:param name="u" select="@u"/>
    <xsl:if test="$ctxt">
        <xsl:choose>
            <xsl:when test="not(/characterMapping/contexts/group[@id=$ctxt])">
                <p>Missing context <xsl:value-of select="$ctxt"/> definition
                   for 0x<xsl:value-of select="$b"/> mapping to U+<xsl:value-of select="$u"/></p>
            </xsl:when>
            <xsl:when test="count(/characterMapping/contexts/group[@id=$ctxt]) > 1">
                <p>Multiple context <xsl:value-of select="$ctxt"/> definitions
                   for 0x<xsl:value-of select="$b"/> mapping to U+<xsl:value-of select="$u"/></p>
            </xsl:when>
        </xsl:choose>
    </xsl:if>
...
<xsl:template match="class-ref" mode="errors">
    <xsl:if test="not(//class[@name=current()/@name])">
        <p>No class definition for <xsl:value-of select="@name"/></p>
    </xsl:if>
</xsl:template>
...
<xsl:template name="range-errors">
    ...
    <xsl:call-template name="test-bctxt">
    	<xsl:with-param name="b" select="$hb"/>
    	<xsl:with-param name="u" select="$hu"/>
    </xsl:call-template>
    <xsl:call-template name="test-uctxt">
    	<xsl:with-param name="b" select="$hb"/>
    	<xsl:with-param name="u" select="$hu"/>
